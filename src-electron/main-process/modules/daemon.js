...
export class Daemon {
    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                ...
                if (!fs.existsSync(evolutiond_path)) { resolve(false) }
    ...
    async checkRemoteHeight () {
        let options = {
            protocol: "https://",
            hostname: "explorer.evolution-network.org",
            endpoint: "/api/networkinfo"
        }
        if(this.testnet) {
            options.hostname = "testnet.evolution-network.org"
        }
    ...
    start (options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (options.app.testnet) {
                this.testnet = true
                args.push("--testnet")
                args.push("--log-file", path.join(options.app.data_dir, "testnet", "logs", "evolutiond.log"))
                //args.push("--add-peer", "45.77.68.151:13310")
            } else {
                args.push("--log-file", path.join(options.app.data_dir, "logs", "evolutiond.log"))
            }
            ...
            if (process.platform === "win32") {
                ...
                this.daemonProcess = child_process.spawn(path.join(__evolution_bin, "evolutiond"), args, {
                    detached: true
                })
            }
